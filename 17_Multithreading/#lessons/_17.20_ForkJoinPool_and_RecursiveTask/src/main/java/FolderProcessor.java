...
/** Пример
 Мы внедрим программу, которая будет искать файлы с определенным расширением внутри папки и ее вложенных папок.
 Класс ForkJoinTask, который мы будем внедрять, будет обрабатывать содержимое папки. Каждая вложенная папка
 внутри этой папки будет отправлять новую задачу ForkJoinPool классу асинхронным способом.
 Для каждого файла внутри этой папки задача проверит расширение файла и добавит его в список результатов,
 если оно будет выполнено.
 */
...
class ForkJoinExample {
  public static void main(String[] args) {
    ...
    FolderProcessor downloads = new FolderProcessor("/home/kedr/Загрузки", "png");
    FolderProcessor images = new FolderProcessor("/home/kedr/Изображения", "png");
    FolderProcessor documents = new FolderProcessor("/home/kedr/Документы", "png");
    ...
    } while ((!downloads.isDone()) || (!images.isDone()) || (!documents.isDone()));
