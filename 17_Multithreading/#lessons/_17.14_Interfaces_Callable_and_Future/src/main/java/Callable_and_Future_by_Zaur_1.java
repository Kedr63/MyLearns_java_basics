 ...
 * вне Runnable, чтобы передать значение result, используем переменную класса RunnableFactorial, а все потому что
 * return type Runnable - void (он ничего не возвращает). Также мы не можем выбросить в нем Exception, а очень хотелось бы.
 * Это недостатки интерфейса Runnable.
  ...
  public static void main(String[] args) {
   ...
   Этот результат хранится в объекте типа Future. И из него результат получим с помощью метода get  */
    try {
      ...
      System.out.println(e.getCause()); /** getCause поможет узнать причину исключения. А что это за причина покажет
