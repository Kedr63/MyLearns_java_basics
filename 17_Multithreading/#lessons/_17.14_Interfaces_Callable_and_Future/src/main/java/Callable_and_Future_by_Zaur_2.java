 ...
 * можем делать cancel нашего таска или узнавать закончилась ли его работа с помощью isDone() Почему
 * здесь уже не надо использовать awaitTermination() метод? Когда мы дойдем до System.out.println(factorialResult3);
 * factorialResult3 уже будет хранить значение, поэтому awaitTermination мы не используем
...
/** Почему здесь уже не надо использовать awaitTermination() метод? Когда мы дойдем до
 * System.out.println(factorialResult3);
 * factorialResult3 уже будет хранить значение, поэтому awaitTermination мы не используем  */
...
/** 📌 Метод submit() мы можем использовать когда работаем с Runnable тоже, но он будет возращать null, т.к. run() ничего не возращает.
 * Какой тогда смысл использовать submit с Runnable? С его помощью мы можем делать cancel нашего таска или узнавать закончилась ли его
 * работа с помощью isDone()  */
