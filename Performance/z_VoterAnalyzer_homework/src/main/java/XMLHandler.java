      ...
      throws SAXException
  {
    try {
      ...
      if (qName.equals("visit") && voter != null) { // если сталкиваемся с тэгом /visit/
        ...
        DBConnection.addVoterToDB(voter.getName(), bd);

       /* int count = voterCounts.getOrDefault(voter,
            0);   // новый метод, вместо contains. Здесь если в коллекции нет voter-а, то вернется
