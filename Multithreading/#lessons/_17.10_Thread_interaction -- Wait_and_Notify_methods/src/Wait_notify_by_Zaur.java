public class Wait_notify_by_Zaur {
  ...
  // Рассмотрим пример: пусть у нас будет магазин, в котором продают хлеб, на ветрине одновременно
  // может находиться не более пяти хлебов. Таким образом если на ветрине пять хлебов, то продавец
  ...
  // ждать пока хлеб не появится. И давайте допустим что в день печется для данного всего 10 хлебов.
...
class Market {
  ...
  // метод симулирования взятие хлеба потребителем
...
// контекста, т.е из synchronized блока или методаю. Данные методы вызываются на объекте, который
...
// что поток оказывается в очень редких случаях проснуться без notify() и тогда мы должны быть уверены что
